...
def get_segmentation_mask(img):
    ...
    if pose_mask is not None:
        mask = cv2.bitwise_or(pose_mask, selfie_mask)
    else:
        mask = selfie_mask
...
def fetch_images_and_height(endpoint_url):
    ...
    return img_front, img_side, height_cm
...
def run_measurement_tool_from_images(img_front, img_side, height_cm):
    ...
    for key, val_px in linear_meas.items():
        results[key] = val_px * scale_inch
